...
def measure_sorts():
    # Délky seznamů, které budeme testovat
    lists_lens = [x for x in range(10, 1500, 300)]
    ...
    # Měření jednotlivých algoritmů
    for x in lists_lens:
        unsorted_list = [random.randint(0, x**2) for _ in range(x)]
    ...
    # Popis os
    plt.xlabel("Délka seznamu (počet prvků)")
    plt.ylabel("Výkon algoritmu (např. doba běhu nebo počet operací)")

    # Logaritmická osa Y
    plt.yscale("log")

    # Titulek a legenda
    plt.title("Porovnání výkonu třídicích algoritmů")
    plt.legend()
